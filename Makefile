...
.PHONY: all
all: dep fmt lint test build ## Runs dep, fmt, lint, test and build.
...
.PHONY: build
build: .build/horcrux-$(GOOS)-$(GOARCH) ## Builds all binaries.
...
.PHONY: install
install: $(GOPATH)/bin/horcrux ## Install all binaries into the $GOPATH/bin directory.
...
.build/horcrux-$(GOOS)-$(GOARCH): $(GOMODDEPS) $(call go-pkg-sourcefiles, $(MOD_NAME)/cmd/horcrux)
	@echo ">> building horcrux production binary for $(GOOS)/$(GOARCH)"
	@GOOS=$(GOOS) GOARCH=$(GOARCH) $(GO) build $(GOFLAGS) -o .build/horcrux-$(GOOS)-$(GOARCH) ./cmd/horcrux

# INSTALL TARGETS

$(GOPATH)/bin/horcrux: $(GOMODDEPS) $(call go-pkg-sourcefiles, $(MOD_NAME)/cmd/horcrux)
