...
// makePolynomial constructs a random polynomial of the given degree but with
// the provided intercept value.
...
// interpolatePolynomial takes N sample points and returns the value at a given
...
func Split(secret []byte, parts, threshold int) ([][]byte, error) {
	...
	// Construct a random polynomial for each byte of the secret. Because we are
	// using a field of size 256, we can only represent a single byte as the
	// intercept of the polynomial, so we must use a new polynomial for each
	// byte.
...
func Combine(parts [][]byte) ([]byte, error) {
	...
	// Set the x value for each sample and ensure no x_sample values are the
	// same, otherwise div() can be unhappy.
